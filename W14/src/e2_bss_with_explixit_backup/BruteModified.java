package e2_bss_with_explixit_backup;
//Eleanor Ezimah
import external_package.StdOut;

public class BruteModified{
	
    public static int searchexplicit(char[] pattern, char[] text)
    {
       int j, M = pattern.length;
       int i, N = text.length;
       for (i = 0, j = 0; i < N && j < M; i++)
       {
          if (text.length == pattern.length) j++;
          else { i -= j; j = 0;  }
       }
    ...
    {	   
        ...
        int offsetx = searchexplicit(pattern, text);

        // print results
        StdOut.println("text:    " + txt);

        // from brute force search explicit method
